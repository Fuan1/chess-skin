...
async function loadCustomSkinData(skinId) {
  return new Promise((resolve) => {
    ...
    chrome.storage.local.get([storageKey], function (result) {
      ...
      console.log(
        `${skinId} 스킨 데이터 로드: ${Object.keys(skinData).length}개 파일`
      );
      resolve(skinData);
    });
  });
...
async function loadSettings() {
  return new Promise((resolve) => {
    chrome.storage.sync.get(
      ["selectedSkin", "skinEnabled", "mapEnabled"],
      function (data) {
        // 설정 값 적용
...
async function generateRules(skinId) {
  ...
  for (let i = 0; i < PIECE_CODES.length; i++) {
    ...
    if (isCustom && customSkinData[fileName]) {
      // 커스텀 스킨인 경우
      const dataURL = getCachedDataURL(
        customSkinData[fileName],
        `${skinId}_${fileName}`
      );

      if (dataURL) {
        rules.push(
          createDataURLRedirectRule(i + 1, dataURL, `*/150/${code}.png`)
        );
      }
    } else {
      // 기본 스킨인 경우
      rules.push(
        createExtensionRedirectRule(
          i + 1,
          `/skins/${skinId}/${code}.png`,
          `*/150/${code}.png`
        )
      );
    }
  ...
  if (state.mapEnabled) {
    if (isCustom) {
      ...
      if (customSkinData["grid.png"]) {
        const dataURL = getCachedDataURL(
          customSkinData["grid.png"],
          `${skinId}_grid.png`
        );

        if (dataURL) {
          rules.push(
            createDataURLRedirectRule(
              PIECE_CODES.length + 1,
              dataURL,
              "*/200.png"
            )
          );
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  console.log(
    `초기 스킨 설정: ${settings.skinId}, 활성화 상태: ${settings.enabled}, 맵 활성화 상태: ${settings.mapEnabled}`
  );
...
function handleApplySkinRequest(request, sendResponse) {
  console.log(
    `스킨 적용 요청: ${request.skin}, 활성화 상태: ${request.enabled}, 맵 활성화 상태: ${request.mapEnabled}`
  );
